...
- [ ] **Any prop-TYPE change is classified by direction — a narrowing is breaking.** For every changed prop type, compare new vs the PREVIOUS type. Widening (old ⊂ new, accepts more) = non-breaking. **Narrowing (new ⊂ old, accepts less) = BREAKING** — a consumer value valid before now fails `tsc`. `React.ReactNode` is the widest; anything tighter (e.g. `IconInput`) narrows it. Do NOT take a changeset's "non-breaking / type widening only" claim at face value — verify with an `expectTypeOf`/`@ts-expect-error` probe using a value valid under the OLD type. This box exists because 0.40.0's F-10 was mislabeled "widening only" while it narrowed 14 `React.ReactNode` props, breaking 3 consumer call sites (devalok-design/shilp-sutra#61).
- [ ] **Breaking-first summary block is present + correct.** `pnpm version-packages` runs `scripts/prepend-breaking-summary.mjs`, which prepends a `> ### ⚠️ Breaking in X.Y.Z` callout to the newest CHANGELOG section for every `feat!`/`**Breaking.` entry. Confirm it fired (or is correctly absent for a non-breaking release) and lists the right entries. It catches MARKED breaks only — a mislabeled narrowing (previous box) will not appear automatically.
- [ ] **Generated CHANGELOG entry reads well.** Changesets concatenates changeset bodies; reorder or tighten if needed BEFORE merge.
- [ ] **llms.txt router + mcp-manifest.json reflect reality.** `build-mcp-manifest.mjs` regenerates both from `docs/components/*.md`; check that any new/changed component's Props + Composability + Changes sections are current.
