...
export function TokenEditor({ primitives, semantic, onChangePrimitive, onChangeSemantic, onResetToken, }) {
    return (_jsxs("div", { className: "space-y-1", children: [_jsx(Accordion, { label: "Color Scales", description: "Pick a base color to auto-generate each scale", defaultOpen: true, hasChanges: Object.keys(primitives).length > 0, children: _jsx("div", { className: "space-y-6 pt-2", children: COLOR_SCALES.map((scale) => {
                ...
                return (_jsx(Accordion, { label: group.label, description: group.description, hasChanges: groupHasChanges, children: _jsx("div", { className: "space-y-3 pt-2", children: group.tokens.map((token) => {
                            ...
                            if (token.type === 'color') {
                                return (_jsxs("div", { className: "flex items-center gap-2", children: [_jsx("label", { className: "w-24 text-xs text-surface-fg-muted", children: token.label }), _jsx("input", { type: "color", value: currentValue.startsWith('#') ? currentValue : '#888888', onChange: (e) => onChangeSemantic(token.name, e.target.value), className: "h-6 w-6 cursor-pointer rounded border border-surface-border" }), _jsx("input", { type: "text", value: currentValue, onChange: (e) => onChangeSemantic(token.name, e.target.value), className: "flex-1 rounded border border-surface-border bg-surface-3 px-2 py-1 text-xs font-mono" }), isOverridden && (_jsx("button", { onClick: () => onResetToken(token.name), className: "text-xs text-surface-fg-subtle hover:text-error-11", children: "\u21BA" }))] }, token.name));
                            }
                            // Size / number controls (slider + input)
                            return (_jsxs("div", { className: "flex items-center gap-2", children: [_jsx("label", { className: "w-24 text-xs text-surface-fg-muted", children: token.label }), _jsx("input", { type: "range", min: token.min ?? 0, max: token.max ?? 100, step: token.step ?? 1, value: parseFloat(currentValue) || 0, onChange: (e) => {
                                            const val = token.unit ? `${e.target.value}${token.unit}` : e.target.value;
                                            onChangeSemantic(token.name, val);
                                        }, className: "flex-1" }), _jsx("span", { className: "w-16 text-right text-xs font-mono text-surface-fg-muted", children: currentValue }), isOverridden && (_jsx("button", { onClick: () => onResetToken(token.name), className: "text-xs text-surface-fg-subtle hover:text-error-11", children: "\u21BA" }))] }, token.name));
                        }) }) }, group.label));
            })] }));
}
