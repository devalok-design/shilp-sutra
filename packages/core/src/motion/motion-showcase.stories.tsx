...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-ds-04">
      <h3 className="text-ds-lg font-semibold text-text-primary border-b border-border pb-ds-02">{title}</h3>
      {children}
    </div>
  )
}
      ...
      <div className="grid grid-cols-3 gap-ds-04" style={{ maxWidth: 600 }}>
        {['Analytics', 'Reports', 'Settings'].map((title) => (
          <Card key={title} interactive>
            ...
            <CardContent>
              <p className="text-ds-sm text-text-secondary">Hover me, click me.</p>
            </CardContent>
          </Card>
        ))}
      </div>
  ...
  return (
    <Section title="Checkbox & Switch — AnimatePresence + spring physics">
      ...
      <div className="flex items-center gap-ds-06">
        <label className="flex items-center gap-ds-03 text-ds-sm text-text-primary cursor-pointer">
      ...
      <MotionStagger key={key} className="flex flex-col gap-ds-02" style={{ maxWidth: 300 }}>
        {items.map((item, i) => (
          <MotionStaggerItem key={i}>
            <div className="flex items-center gap-ds-03 rounded-ds-md bg-layer-02 p-ds-03">
              <IconCheck className="h-ico-sm w-ico-sm text-success" />
              <span className="text-ds-sm text-text-primary">{item}</span>
            </div>
          </MotionStaggerItem>
        ))}
      </MotionStagger>
    ...
    <Section title="Spring Physics — feel the difference">
      ...
      <div className="grid grid-cols-4 gap-ds-04" style={{ maxWidth: 500 }}>
        {(Object.entries(springs) as [string, object][]).map(([name, config]) => (
          ...
          >
            <span className="text-ds-xs font-semibold text-text-secondary">{name}</span>
            <AnimatePresence mode="wait">
              {active === name && (
                <motion.div
                  ...
                  exit={{ scale: 0 }}
                  transition={config}
                  className="h-16 w-16 rounded-ds-md bg-interactive"
                />
              )}
            </AnimatePresence>
            {active !== name && (
              <div className="h-16 w-16 rounded-ds-md border-2 border-dashed border-border" />
            )}
          </button>
        ))}
      </div>
...
function FullShowcase() {
  return (
    <div className="flex flex-col gap-ds-08 p-ds-06" style={{ maxWidth: 800 }}>
      <div>
        <h1 className="text-ds-2xl font-bold text-text-primary">Framer Motion Showcase</h1>
        <p className="mt-ds-02 text-ds-md text-text-secondary">
