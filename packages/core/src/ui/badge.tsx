...
const badgeVariants = cva(
  'inline-flex items-center gap-ds-02b font-sans font-medium rounded-ds-full border',
  {
    ...
    compoundVariants: [
      ...
      { variant: 'subtle', color: 'accent', className: 'bg-accent-2 text-accent-9 border-accent-9' },
  ...
  ({ className, variant, color, size, dot, onDismiss, children, ...props }, ref) => {
    return (
      <span ref={ref} className={cn(badgeVariants({ variant, color, size }), className)} {...props}>
        {dot && (
          <span
            className="h-ds-02b w-ds-02b rounded-ds-full bg-current opacity-70 shrink-0 animate-pulse-ring"
            aria-hidden="true"
          />
        )}
        {children}
