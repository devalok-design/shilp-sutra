  ...
  ({ className, variant = 'inline', children, ...props }, ref) => {
    if (variant === 'block') {
      return (
        <pre
          ...
          className={cn(
            'overflow-x-auto rounded-ds-md border border-surface-border-strong bg-surface-2 p-ds-05 font-mono text-ds-sm leading-ds-relaxed text-surface-fg',
            className,
          )}
          {...props}
        >
    ...
    return (
      <code
        ref={ref}
        className={cn(
          'rounded-ds-sm bg-surface-3 border border-surface-border px-ds-02 py-ds-01 font-mono text-ds-sm text-surface-fg',
          className,
        )}
        {...props}
      >
