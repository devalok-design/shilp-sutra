...
const meta: Meta<typeof Combobox> = {
  title: 'Components/Selectors/Combobox',
  component: Combobox,
  ...
  render: () => {
    ...
    const renderOption = (option: ComboboxOption, _selected: boolean) => (
      <span className="flex items-center gap-ds-03">
        <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-pill bg-accent-2 text-ds-sm font-medium text-accent-11">
          {option.label
            .split(' ')
            .map((part) => part[0])
            .join('')
