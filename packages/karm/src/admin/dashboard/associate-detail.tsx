    ...
    <div ref={ref} className={cn("relative flex items-center justify-between md:items-stretch", className)} {...props}>
      ...
      {isFutureDate ? (
        ...
        <div className="flex w-full flex-col items-center justify-center p-ds-06">
          ...
          <div className="flex w-full items-center justify-between rounded-ds-lg border border-surface-border-strong bg-surface-raised px-ds-05 max-md:h-ds-lg">
            <input
                  ...
                  <div
                    ...
                    onKeyDown={(e) => {
                      if (e.altKey && e.key === 'ArrowUp' && idx > 0) {
                        e.preventDefault()
                        onReorderTasks?.(task.id, userTasks[idx - 1].id)
                      } else if (e.altKey && e.key === 'ArrowDown' && idx < userTasks.length - 1) {
                        e.preventDefault()
                        onReorderTasks?.(task.id, userTasks[idx + 1].id)
                      }
                    }}
