...
const meta: Meta<typeof TaskProperties> = {
  ...
  decorators: [
    (Story) => (
      <div className="w-[400px] rounded-[var(--radius-lg)] border border-[var(--color-border-default)] bg-[var(--color-layer-01)] p-4">
        <Story />
      </div>
    ),
  ],
  ...
  render: (args) => (
    <div className="space-y-6">
      {(['LOW', 'MEDIUM', 'HIGH', 'URGENT'] as const).map((priority) => (
        <div key={priority} className="rounded-[var(--radius-lg)] border border-[var(--color-border-default)] bg-[var(--color-layer-01)] p-4">
