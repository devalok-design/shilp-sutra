  ...
  render: () => {
    ...
    const renderOption = (option: ComboboxOption, _selected: boolean) => (
      <span className="flex items-center gap-ds-03">
        <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[var(--color-interactive-subtle)] text-ds-sm font-medium text-[var(--color-interactive)]">
          {option.label
            .split(' ')
            .map((part) => part[0])
            .join('')
