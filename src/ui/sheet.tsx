...
const sheetVariants = cva(
  'fixed z-[var(--z-modal)] gap-4 bg-[var(--color-layer-01)] p-6 shadow-[var(--shadow-05)] transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out',
  {
    variants: {
      side: {
...
>(({ side = 'right', className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content
      ...
      {...props}
    >
      <SheetPrimitive.Close className="absolute right-4 top-4 rounded-sm opacity-70 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-[var(--color-focus)] focus:ring-offset-2 disabled:pointer-events-none">
        <X className="h-4 w-4" />
